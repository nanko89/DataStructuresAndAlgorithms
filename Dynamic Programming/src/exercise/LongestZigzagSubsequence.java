package exercise;

import java.util.*;

public class LongestZigzagSubsequence {
    public static int[][] dp;

    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);

        int[] numbers = Arrays.stream(scanner.nextLine().split("\\s+")).mapToInt(Integer::parseInt).toArray();
        dp = new int[numbers.length + 1][2];
        int[][] prev = new int[numbers.length + 1][2];

        dp[0][0] = 1;
        dp[0][1] = 1;

        prev[0][0] = -1;
        prev[0][1] = -1;

        int maxLength = 0;
        int[] bestIndex = new int[2];

        for (int currentIndex = 0; currentIndex < numbers.length; currentIndex++) {
            int currentNumber = numbers[currentIndex];

            for (int prevIndex = currentIndex - 1; prevIndex >= 0; prevIndex--) {
                int prevNumb = numbers[prevIndex];

                if (currentNumber > prevNumb && dp[currentIndex][0] <= dp[prevIndex][1] + 1) {
                    dp[currentIndex][0] = dp[prevIndex][1] + 1;
                    prev[currentIndex][0] = prevIndex;
                }else if (currentNumber < prevNumb && dp[currentIndex][1] <= dp[prevIndex][0] + 1) {
                    dp[currentIndex][1] = dp[prevIndex][0] + 1;
                    prev[currentIndex][1] = prevIndex;
                }
            }

            if (maxLength < dp[currentIndex][0]) {
                maxLength = dp[currentIndex][0];
                bestIndex[0] = currentIndex;
                bestIndex[1] = 0;
            } else if (maxLength < dp[currentIndex][1]) {
                maxLength = dp[currentIndex][1];
                bestIndex[0] = currentIndex;
                bestIndex[1] = 1;
            }
        }

        Deque<Integer> zigZag = new ArrayDeque<>();

        int beginRow = bestIndex[0];

        while (beginRow >= 0) {
            zigZag.push(numbers[beginRow]);
            beginRow = prev[beginRow][bestIndex[1]];
            bestIndex[1] = bestIndex[1] == 0 ? 1 : 0;
        }

        while (!zigZag.isEmpty()) {
            System.out.print(zigZag.pop() + " ");
        }
    }
}
